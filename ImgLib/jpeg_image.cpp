...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    row_stride = image.GetHeight() * 3; /* JSAMPLEs per row in image_buffer */
    std::vector<JSAMPLE> buffer(row_stride);

    while (cinfo.next_scanline < cinfo.image_height) {
        const Color* image_begin = image.GetLine(cinfo.next_scanline);
        const Color* image_end = image_begin + image.GetWidth();

        int i = 0;

        for (; image_begin < image_end; ++image_begin) {
            buffer[i] = static_cast<JSAMPLE>(image_begin->r);
            buffer[i + 1] = static_cast<JSAMPLE>(image_begin->g);
            buffer[i + 2] = static_cast<JSAMPLE>(image_begin->b);
